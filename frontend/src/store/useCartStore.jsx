...
export const useCartStore = create((set,get) => ({
    ...
    getCartItems : async () => {
        try {
            const res = await axios.get('/api/cart/getCartProducts') ; 
            get().calculateTotals() ;
            console.log(res) ; 
   ...
   addToCart: async (product) => {
    try {
        console.log(product); 
        const res = await axios.post('/api/cart/addToCart', { productId: product._id });
        
        set((prevState) => {
            const existingItem = prevState.cart.find((item) => item?.product?._id === product._id);
            let newCart = [];
            if (existingItem) {
                console.log("gello i am here") ; 
...
updateQuantity : async (productId, quantity) => {
    ...
    calculateTotals : () => {
        ...
        const subtotal = cart.reduce((sum,item) => sum + item.product.prix * item.quantity ,0 ) ; 
        let total = subtotal ;
        console.log(total) ;  
        if(coupon) {
